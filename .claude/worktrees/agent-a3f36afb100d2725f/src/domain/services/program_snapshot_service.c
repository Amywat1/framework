                                                    ...
                                                    const char *program_id)
{
    ...
    if (program_snapshot_service_args == 0 || program_snapshot_service_args->program_snapshot == 0 ||
        program_snapshot_service_args->wash_program == 0 ||
        program_snapshot_service_args->program_repository_port == 0 || program_id == 0)
    {
    ...
    repository_result = program_snapshot_service_args->program_repository_port->validate_program_snapshot(
        program_snapshot_service_args->program_repository_port->context, program_id,
        program_snapshot_service_args->program_snapshot, program_snapshot_service_args->wash_program);
    ...
    program_snapshot_capture(program_snapshot_service_args->program_snapshot, program_id,
                             program_snapshot_service_args->wash_program->revision,
                             program_snapshot_service_args->current_time_ms,
                             program_snapshot_service_args->wash_program, PROGRAM_SNAPSHOT_VALIDATION_VALID);
