 ...
 * 互斥量初始化（优先级继承）
 *
 * safety_thread 以 SCHED_FIFO 优先级 90 调用 event_publish()，而队列锁
 * 同时被 9 个 SCHED_OTHER 周期任务竞争。若普通优先级线程持锁期间被抢占，
 * 急停线程会阻塞在互斥量上且阻塞时长不受优先级保护，构成无界优先级反转。
 * 因此两把锁均启用 PTHREAD_PRIO_INHERIT。
 *
