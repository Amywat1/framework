    ...
    *out_engine = NULL;
...
#if PROGRAM_INTEGRITY_CHECK
    {
        ...
        if (engine_program_manifest_verify(prog_path, manifest_path, manifest_err, (unsigned)sizeof(manifest_err))
            != SW_OK) {
            LOG_ERROR("wash_worker: manifest verify failed json=[%s] err=[%s]", prog_path, manifest_err);
            return SW_ERR_CRC;
        }
    }
#endif

    {
        ...
        if (prog == NULL) {
            LOG_ERROR("wash_worker: load program failed path=[%s] err=[%s]", prog_path, err_buf);
            return SW_ERR_PARAM;
        }
    }
...

        {
...

            {
                ...
                if ((final_state == ENGINE_STATE_DONE) && (stored_abort < 0) && !timed_out) {
                    ...
                    LOG_WARN("wash_worker: wash aborted cause=%d engine_state=%d", (int)cause, (int)final_state);
                }
            }
        }
...
sw_err_t wash_orchestrator_init(void)
{
    ...
    sw_err_t ret = thread_register("wash_worker", wash_worker_fn, SCHED_OTHER, 0, THD_WASH_WORKER_STACK);
    if (ret != SW_OK) {
        return ret;
    }
