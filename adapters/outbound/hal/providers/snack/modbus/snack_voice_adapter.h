...
#ifndef ADAPTERS_OUTBOUND_HAL_PROVIDERS_SNACK_MODBUS_SNACK_VOICE_ADAPTER_H
#define ADAPTERS_OUTBOUND_HAL_PROVIDERS_SNACK_MODBUS_SNACK_VOICE_ADAPTER_H
...
/**
 * @brief  注册 snack_voice_adapter 操作集到 hal_voice_port，同时存储连接参数供 init 使用。
 * @param  serial_port  Modbus RTU 串口路径（如 "/dev/ttyS1"）
 * @param  baud         串口波特率
 * @param  modbus_addr  Modbus 从站地址（1~247）
 * @note   不读取配置、不初始化硬件；真实 drv_voice_init() 只在 hal_voice.init() 中执行。
 */
...
#endif /* ADAPTERS_OUTBOUND_HAL_PROVIDERS_SNACK_MODBUS_SNACK_VOICE_ADAPTER_H */
