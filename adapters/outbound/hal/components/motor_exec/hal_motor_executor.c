...
static sw_err_t drv_set_output(motor_driver_t *d, hal_motor_speed_t speed, hal_motor_dir_t dir)
{
...
static bool sensor_limit(motor_executor_t *e, int i, hal_motor_limit_kind_t k)
{
...
static hal_motor_cmd_result_t cmd_make(hal_motor_cmd_status_t st, const char *reason)
{
...
static hal_motor_cmd_result_t cmd_reject(const char *reason)
{
 ...
 */
static bool speed_valid(motor_executor_t *e, int i, hal_motor_speed_t speed)
{
    if (speed.value <= 0) {
        return false;
    }
    if (speed.kind == HAL_MOTOR_SPEED_FREQ) {
        return true;
    }
    return (speed.kind == HAL_MOTOR_SPEED_GEAR) && (speed.value <= e->cfg.motors[i].gear_count);
}
 ...
 */
static hal_motor_cmd_result_t apply_goal(motor_executor_t *e, int i, const motor_pending_cmd_t *pc)
{
    motor_mstate_t *s = &e->m[i];
...
static void complete_move(motor_executor_t *e, int i, hal_motor_event_type_t type, hal_motor_end_condition_t trig)
{
    motor_mstate_t *s = &e->m[i];
...
static void motor_tick(motor_executor_t *e)
{
    e->now = clock_now(e);
...
static motor_init_result_t motor_init(motor_executor_t *e, const motor_config_t *cfg, const motor_ports_t *ports)
{
    e->cfg    = *cfg;
    e->ports  = *ports;
    e->cb     = NULL;
...
static motor_init_result_t motor_reinit(motor_executor_t *e)
{
    return do_init(e);
}
 ...
 */
static hal_motor_cmd_result_t cmd_guard(motor_executor_t *e, int i, unsigned flags)
{
    if (e->in_dispatch) {
    ...
    if ((flags & MOTOR_CMD_REJECT_FAULT) && (e->m[i].phase == HAL_MOTOR_PHASE_FAULT)) {
        return cmd_reject("fault");
    }
    return cmd_make(HAL_MOTOR_CMD_ACCEPTED, "");
}

static hal_motor_cmd_result_t motor_run(motor_executor_t            *e,
                                        int                          i,
                                        hal_motor_speed_t            spd,
                                        hal_motor_dir_t              dir,
                                        const hal_motor_move_spec_t *spec)
{
    hal_motor_cmd_result_t g  = cmd_guard(e, i, MOTOR_CMD_NEED_NOW | MOTOR_CMD_REJECT_SAFETY | MOTOR_CMD_REJECT_FAULT);
    motor_pending_cmd_t    pc = {0};

    if (!hal_motor_cmd_ok(g)) {
        return g;
    }
...
static hal_motor_cmd_result_t motor_stop(motor_executor_t *e, int i)
{
    hal_motor_cmd_result_t g = cmd_guard(e, i, MOTOR_CMD_NEED_NOW);
    ...
    case HAL_MOTOR_PHASE_WAITING_START:
        s->queued = false;
        s->phase  = HAL_MOTOR_PHASE_STOPPED;
        return cmd_make(HAL_MOTOR_CMD_ACCEPTED, "queue-cancel");
    case HAL_MOTOR_PHASE_REVERSAL_WAIT:
        s->phase          = HAL_MOTOR_PHASE_STOPPED;
        s->cooldown_until = e->now + e->cfg.motors[i].cooldown_ms;
        return cmd_make(HAL_MOTOR_CMD_ACCEPTED, "reversal-cancel");
    case HAL_MOTOR_PHASE_RUNNING:
    case HAL_MOTOR_PHASE_DECELERATING: {
        ...
        for (n = 0U; n < (sizeof(k_order) / sizeof(k_order[0])); ++n) {
            ...
            if (hal_motor_limit_mask_has(s->spec.limit_mask, kind) && sensor_limit(e, i, kind)) {
                ...
                dispatch(e);
                return cmd_make(HAL_MOTOR_CMD_ACCEPTED, "arrived-on-limit");
            }
        }
        ...
        s->move_active       = false;
        s->phase             = HAL_MOTOR_PHASE_DECELERATING;
        return cmd_make(HAL_MOTOR_CMD_ACCEPTED, "stopping");
    }
    default:
        return cmd_make(HAL_MOTOR_CMD_ACCEPTED, "already-stopped");
    }
}

static hal_motor_cmd_result_t motor_home(motor_executor_t *e, int i)
{
    hal_motor_cmd_result_t g = cmd_guard(e, i, MOTOR_CMD_NEED_NOW | MOTOR_CMD_REJECT_SAFETY | MOTOR_CMD_REJECT_FAULT);
    hal_motor_move_spec_t  spec = {0};
    int                    freq;
    ...
    spec.limit_mask = HAL_MOTOR_LIMIT_MASK_ORIGIN;
    return motor_run(e, i, hal_motor_speed_freq(freq), HAL_MOTOR_DIR_REVERSE, &spec);
}

static hal_motor_cmd_result_t motor_zero_encoder(motor_executor_t *e, int i)
{
    hal_motor_cmd_result_t g = cmd_guard(e, i, 0);
    ...
    for (int t = 0; t < MOTOR_ZERO_MAX_TRIES; ++t) {
        if (enc_zero(enc)) {
            ...
            return cmd_make(HAL_MOTOR_CMD_ACCEPTED, "zeroed");
        }
    }
    push_event(e, i, HAL_MOTOR_EVENT_WARNING, HAL_MOTOR_END_NONE, HAL_MOTOR_FAULT_ENCODER_SIGNAL);
    return cmd_reject("zero-failed");
}

static hal_motor_cmd_result_t motor_confirm_baseline(motor_executor_t *e, int i)
{
    if (bad_motor(e, i)) {
        return cmd_reject("bad-motor");
    }
    e->m[i].baseline_trusted = true;
    return cmd_make(HAL_MOTOR_CMD_ACCEPTED, "baseline-confirmed");
}

static void motor_reset_estop(motor_executor_t *e)
{
    if (e->in_dispatch) {
        return;
    }
    ...
    e->estop_latched = false;
    for (int i = 0; i < e->motor_count; ++i) {
        if (e->m[i].phase == HAL_MOTOR_PHASE_ESTOP) {
            e->m[i].phase = HAL_MOTOR_PHASE_STOPPED;
        }
    }
    e->now = clock_now(e);
...
static void motor_reset_watchdog(motor_executor_t *e)
{
    if (e->in_dispatch) {
        return;
    }
    ...
    e->last_tick_valid = true;
    for (int i = 0; i < e->motor_count; ++i) {
        if (e->m[i].fault_code == HAL_MOTOR_FAULT_WATCHDOG) {
            e->m[i].phase      = HAL_MOTOR_PHASE_STOPPED;
            e->m[i].fault_code = HAL_MOTOR_FAULT_NONE;
        }
    }
...
static hal_motor_cmd_result_t motor_recover(motor_executor_t *e, int i, hal_motor_recovery_step_t step)
{
    hal_motor_cmd_result_t g = cmd_guard(e, i, 0);
    ...
    if (step == HAL_MOTOR_RECOVERY_DRIVER_RESET) {
        if (!drv_reset(motor_drv(e, i))) {
            push_event(e, i, HAL_MOTOR_EVENT_FAULT, HAL_MOTOR_END_NONE, s->fault_code);
            return cmd_reject("driver-reset-failed");
        }
        s->driver_reset_done = true;
        return cmd_make(HAL_MOTOR_CMD_ACCEPTED, "driver-reset");
    }
    /* MODULE_STOP */
    ...
    e->now               = clock_now(e);
    s->phase             = HAL_MOTOR_PHASE_STOPPED;
    s->fault_code        = HAL_MOTOR_FAULT_NONE;
    s->driver_reset_done = false;
    s->cooldown_until    = e->now;
    return cmd_make(HAL_MOTOR_CMD_ACCEPTED, "recovered");
}

/* ------------------------- 查询 ------------------------- */

static hal_motor_phase_t motor_phase(const motor_executor_t *e, int i)
{
    if (bad_motor(e, i)) {
...
static int64_t motor_position(const motor_executor_t *e, int i)
{
    if (bad_motor(e, i)) {
        return 0;
    }
...
static int motor_current_freq(const motor_executor_t *e, int i)
{
    ...
    if ((state->phase != HAL_MOTOR_PHASE_RUNNING) || (state->speed.kind != HAL_MOTOR_SPEED_FREQ)) {
        return 0;
    }
    return state->speed.value;
}

static hal_motor_dir_t motor_direction(const motor_executor_t *e, int i)
{
    if (bad_motor(e, i)) {
...
static hal_motor_fault_code_t motor_fault_code(const motor_executor_t *e, int i)
{
    if (bad_motor(e, i)) {
...
static bool motor_baseline_trusted(const motor_executor_t *e, int i)
{
    if (bad_motor(e, i)) {
        return false;
    }
...
static bool motor_encoder_healthy(const motor_executor_t *e, int i)
{
    if (bad_motor(e, i)) {
        return false;
    }
...
static bool motor_in_safe_state(const motor_executor_t *e)
{
