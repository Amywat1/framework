...
static bool validate_step_schema(const cJSON *node, const char *path, char *err, unsigned errsz)
{
    ...
    static const char *const done_allowed[]    = {"type", "signal", "state", "timeout_ms", "confirm_ms"};
    const cJSON             *child;
    char                     child_path[320];
...
static bool validate_program_schema(const cJSON *root, char *err, unsigned errsz)
{
...
engine_program_t *engine_program_load_json_string(const char *json, char *err, unsigned errsz)
{
    ...
    if (!validate_program_schema(root, werr, wsz)) {
        cJSON_Delete(root);
        return NULL;
    }
