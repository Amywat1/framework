...
**版本**：v1.0  
**状态**：已落地（`wash-device-framework` 物模型核心 + 端口契约；MQTT 适配器 / 上报调度器待完整框架迁入）  
**最后同步代码**：2026-07-12（`cloud_point_*`；dispatch 单测 8 用例）  
**适用范围**：`wash-device-framework/cloud/`、`wash-device-framework/ports/**/cloud/`  
...
```text
  ...
  cloud_point_to_json() / cloud_point_to_json_filtered()
        │
        ▼
  cloud_report_port.publish_properties[_delta]()  ← report_scheduler（待迁入）
        │
        ▼
...
| 职责 | 决定方 |
|------|--------|
| 物模型表内容 | 项目 `cloud_model.c` 或等价文件 |
| MQTT Topic / 凭证 | deploy_store + link 适配器 |
| 上报时机与节流 | report_scheduler（完整框架 application 层） |
...
| 单元测试 | ✅ validate(9) / dispatch(8) / watcher(3) / ports(4) |
| MQTT link 适配器 | ❌ 未迁入 |
| cloud_property 适配器 | ❌ 未迁入 |
| report_scheduler | ❌ 未迁入 |
| Demo wiring / 完整物模型表 | ❌ 未接入 |
| 与 command_gateway 联调 | ⚠️ 测试层分别覆盖；端到端 wiring 待完整框架 |
...
- `doc/报警系统设计说明.md` — 与命令/安全域正交；云端上报报警态待 projection 迁入
- `doc/事件总线设计说明.md` — `EVT_CLOUD_*` 分发语义
- `doc/参数存储设计说明.md` — deploy_store（MQTT 凭证等）
