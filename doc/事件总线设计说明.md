...
**版本**：v1.2  
**状态**：已落地（核心实现 + scheduler；Demo bootstrap 待接入）  
**最后同步代码**：2026-07-12（业务订阅：`command_gateway`、`op_mode_bridge`、`safety_posture` 等）  
...
完整 `Project/framework` 在 `bootstrap` 中注册名为 `event_dispatch` 的线程，入口调用 `event_bus_dispatch_loop()`。`wash-device-framework` 单元测试由测试代码手动启动该线程。
...
| 业务订阅（gateway / bridge / posture） | ✅ 单测手动接入 |
| Demo bootstrap + 生产 dispatch 线程 | ❌ 未接入 |
...
- `tests/reports/event_bus.md` — 单元测试报告
- `Project/framework/doc/软件设计框架说明.md` — 运行模型、急停双通道
