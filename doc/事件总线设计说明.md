...
**版本**：v1.1  
**状态**：已落地（`wash-device-framework` 核心实现；业务发布方与 dispatch 线程由完整框架接入）  
**最后同步代码**：2026-07-10（`event_bus_config.h` 独立容量配置；HW 高优先级路由含 IO 在线/离线）  
**适用范围**：`wash-device-framework/`、`Project/framework/framework/runtime/event_bus/`  
**架构基线**：`Project/framework/doc/软件设计框架说明.md` §4.2、§11、§12.4  
...
event_bus_init()
    ↓
wiring()（端口注册；可在此或之后批量 event_subscribe）
    ↓
业务模块 init()
    ↓
启动 event_dispatch 线程（完整框架由 bootstrap 注册）
```

`wash-device-framework` 当前 Demo 仅完成 `event_bus_init()`；dispatch 线程与业务订阅在单元测试或完整框架 bootstrap 中接入。
...
### 6.1 dispatch 线程（完整框架）

完整 `Project/framework` 在 `bootstrap` 中注册名为 `event_dispatch` 的线程，入口调用 `event_bus_dispatch_loop()`。以下模块通过 `event_subscribe_table()` 在启动期注册：

| 订阅方 | 典型订阅事件 |
|--------|--------------|
| `op_mode_bridge` | `EVT_WASH_*`、`EVT_HW_ESTOP_*`、`EVT_SAFETY_LOCKOUT` 等 |
| `safety_supervisor` | 安全与报警相关事件 |
| `command_gateway` | `EVT_CMD_GATEWAY_WAKE` |
| `recovery_service` | 恢复流程事件 |
| `emergency_handler` | 急停与安全归位 |
| `mode_projection` | 模式投影刷新 |
| `safety_posture` | 报警触发/清除（少数 domain 订阅方） |
...
| Demo `event_bus_init()` | ✅ 已调用 |
| Demo dispatch 线程 / 业务订阅 | ❌ 未接入 |
| `runtime/scheduler/` | ❌ 未迁入 |
| 单元测试 | ✅ `test_event_bus`（8 用例，见 `tests/reports/event_bus.md`） |
| 单测未覆盖 | 高优先级 IO 路由、fatal 回调、`event_subscribe_table` |

---

## 12. 相关文档

- `doc/参数存储设计说明.md` — 运行期 KV 存储（与总线正交）
- `tests/reports/event_bus.md` — 单元测试报告
- `Project/framework/doc/软件设计框架说明.md` — 运行模型、事件总线适用边界、急停双通道
- `Project/framework/doc/报警系统设计说明.md` — 报警事件链与 `EVT_ALARM_*` 消费方
- `Project/framework/doc/运行模式状态机设计说明.md` — `EVT_OP_MODE_*` 与模式仲裁
- `Project/framework/doc/云端通信模块设计说明.md` — `EVT_CLOUD_*` 与上报调度
