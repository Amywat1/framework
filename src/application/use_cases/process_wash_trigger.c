...
static wash_execution_service_args_t build_execution_service_args(system_context_t *system_context)
{
    ...
    wash_execution_service_args.sensor_port = &system_context->sensor_port;
...
static operation_result_t advance_or_finish_session(system_context_t *system_context)
{
    ...
    if (system_context->wash_execution.execution_state == EXECUTION_STATE_ABORTED
        && wash_session_is_running(&system_context->wash_session)) {
        ...
        result = wash_session_state_machine_abort(&wash_session_service_args,
            map_execution_abort_result(&system_context->wash_execution),
            system_context->wash_execution.reason_code[0] != '\0'
                ? system_context->wash_execution.reason_code
                : system_context->wash_session.abort_reason,
    ...
    if (system_context->wash_execution.execution_state != EXECUTION_STATE_COMPLETED
        || system_context->wash_execution.lifecycle_state != SEGMENT_LIFECYCLE_COMPLETED) {
        return operation_result_ok();
    }

    if (system_context->wash_execution.segment_index + 1
        < system_context->program_snapshot.frozen_program.segment_count) {
...
static operation_result_t handle_start(system_context_t *system_context, const wash_trigger_event_t *wash_trigger_event)
{
    ...
    system_context->wash_execution.segment_index = -1;
