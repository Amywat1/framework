...
/**
 * @brief 从 control_context 构建程序快照服务参数。
 * @return 程序快照服务参数。
 */
...
/**
 * @brief 从 control_context 构建工步执行服务参数。
 * @return 工步执行服务参数。
 */
                                   ...
                                   const char *result_code, const char *reason_code)
{
    ...
    {
        control_outcome_recorder_set_latest_result(result_code, reason_code);
        return;
    }

    wash_control_outcome_projection_init(&wash_control_outcome_projection);
    wash_control_outcome_projection_set_latest_result(&wash_control_outcome_projection, result_code, reason_code);
    wash_control_outcome_projection_set_transition(
        &wash_control_outcome_projection, TRANSITION_ENTITY_REQUEST,
        wash_trigger_event->trigger_id[0] != '\0' ? wash_trigger_event->trigger_id : "internal-trigger",
        wash_trigger_event->trigger_type, wash_state_text_device_state(previous_device_state),
        wash_state_text_device_state(current_device_state), result_code, reason_code);
 ...
 */
static operation_result_t finalize_segment_or_continue_program(void)
{
    ...
    if (wash_execution->execution_state == EXECUTION_STATE_ABORTED && wash_session_is_running(wash_session))
    {
        ...
        if (result.ok)
        {
            control_context_private_apply_device_state_outcome(
    ...
    if (wash_execution->segment_index + 1 < program_snapshot->frozen_program.segment_count)
    {
        ...
        if (result.ok)
        {
            control_outcome_recorder_set_latest_result(wash_execution_fact.result_code, wash_execution_fact.reason_code);
        }
        return result;
    }
    ...
    if (result.ok)
    {
        control_context_private_apply_device_state_outcome(DEVICE_STATE_IDLE, "completed",
                                                           "program_finished");
    }
    return result;
}
...
static operation_result_t handle_homing(void)
{
    ...
    wash_session = control_context_private_wash_session();
    actuator_port = control_context_private_actuator_port();
    sensor_port = control_context_private_sensor_port();
    ...
    control_context_private_apply_device_state_outcome(DEVICE_STATE_RECOVERING, 0, 0);
    ...
    if (!result.ok || control_context_private_global_fault_present())
    {
        control_context_private_apply_device_state_outcome(DEVICE_STATE_EXCEPTION, 0, 0);
        project_trigger_result(&wash_trigger_event, DEVICE_STATE_STOPPED, DEVICE_STATE_EXCEPTION,
                               ...
                               failure_reason_code != 0 ? failure_reason_code
                                                        : (control_context_private_global_fault_present()
                                                               ? "recovering_alarm_triggered"
                                                               : "homing_failed"));
        return !result.ok ? result : operation_result_fail(ERROR_CODE_INVALID_STATE);
    }

    control_context_private_apply_device_state_outcome(DEVICE_STATE_IDLE, 0, 0);
...
static operation_result_t handle_start(const wash_trigger_event_t *wash_trigger_event)
{
    ...
    wash_session = control_context_private_wash_session_mutable();
    ...
    result = control_context_private_apply_start_accepted(wash_session, wash_execution);
    if (result.ok)
    {
...
static operation_result_t handle_stop(const wash_trigger_event_t *wash_trigger_event)
{
    ...
    if (result.ok)
    {
        control_context_private_apply_device_state_outcome(DEVICE_STATE_STOPPED, "aborted",
...
static operation_result_t handle_fault(const wash_trigger_event_t *wash_trigger_event)
{
    ...
    if (clear_requested)
    {
        ...
        control_context_private_clear_global_fault();
        control_context_private_apply_device_state_outcome(DEVICE_STATE_STOPPED, 0, 0);
    ...
    if (!wash_session_is_running(wash_session))
    {
        control_context_private_set_global_fault(wash_trigger_event->signal_code,
    ...
    if (result.ok)
    {
        control_context_private_apply_device_state_outcome(DEVICE_STATE_EXCEPTION, "aborted",
                                                           wash_trigger_event->signal_code);
    }
    return result;
}
...
static operation_result_t handle_timeout(void)
{
    ...
    wait_condition = control_context_private_wait_condition_mutable();
    if (wait_condition == 0)
    {
        return operation_result_fail(ERROR_CODE_INVALID_STATE);
    }

    result = wait_timeout_service_handle_timeout(wait_condition, control_context_current_time_ms(),
                                                 &wait_timeout_fact);
    if (!result.ok)
    {
        return result;
    ...
    {
        control_outcome_recorder_set_latest_result(wash_execution_fact.result_code, wash_execution_fact.reason_code);
        return finalize_segment_or_continue_program();
    }
    return result;
}
...
/**
 * @brief 在运行中的洗车会话内推进程序工步执行一步，并按需切换工步或结束会话。
...
operation_result_t advance_wash_session_program(void)
{
    ...
    wash_execution_service_args = build_execution_service_args();
    result = wash_execution_service_advance_segment(&wash_execution_service_args, &wash_execution_fact);
    if (!result.ok)
    {
        return result;
    }
