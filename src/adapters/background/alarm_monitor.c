#include "adapters/background/alarm_monitor.h"
 ...
 */
static operation_result_t alarm_monitor_read_snapshot(const sensor_port_t *sensor_port,
                                                       sensor_snapshot_t *sensor_snapshot)
{
 ...
 */
static bool alarm_snapshot_mailbox_init(alarm_snapshot_mailbox_t *snapshot_mailbox)
{
 ...
 */
static bool alarm_snapshot_mailbox_publish(alarm_snapshot_mailbox_t *snapshot_mailbox,
                                          const sensor_snapshot_t *sensor_snapshot, unsigned long occurred_at_ms)
{
 ...
 */
static bool alarm_snapshot_mailbox_try_read(alarm_snapshot_mailbox_t *snapshot_mailbox,
                                             unsigned long *last_consumed_sequence, sensor_snapshot_t *sensor_snapshot,
                                             unsigned long *occurred_at_ms)
{
...
/**
 * @brief 报警监控 IO 线程入口，仅负责周期读取并发布最新快照。
 * @param run_ctx 运行控制句柄。
 * @param context 监控实例。
 */
static void alarm_monitor_io_entry(worker_run_ctx_t *run_ctx, void *context)
{
    alarm_monitor_t *monitor;
    sensor_snapshot_t sensor_snapshot;
    operation_result_t result;

    monitor = (alarm_monitor_t *)context;
    if (monitor == 0)
    {
        return;
    }
    ...
    {
        result = alarm_monitor_read_snapshot(monitor->sensor_port, &sensor_snapshot);
        if (result.ok &&
            alarm_snapshot_mailbox_publish(&monitor->snapshot_mailbox, &sensor_snapshot,
...
/**
 * @brief 报警监控检测线程入口，只消费新快照并执行告警检测。
 * @param run_ctx 运行控制句柄。
 * @param context 监控实例。
 */
static void alarm_monitor_detect_entry(worker_run_ctx_t *run_ctx, void *context)
{
    ...
    while (!worker_run_ctx_stop_requested(run_ctx))
    {
        if (alarm_snapshot_mailbox_try_read(&monitor->snapshot_mailbox, &last_consumed_sequence, &sensor_snapshot,
                                            &occurred_at_ms))
        {
            (void)alarm_detect_job_process_snapshot(&monitor->alarm_evaluator, &sensor_snapshot, occurred_at_ms);
            continue;
        }
...
operation_result_t alarm_monitor_create(alarm_monitor_t **monitor, const alarm_monitor_config_t *config)
{
    alarm_monitor_t *created_monitor;
    ...
    *monitor = 0;
    if (config->settings.enabled &&
        (config->sensor_port == 0 || config->sensor_port->read_snapshot == 0 ||
         config->settings.io_sample_period_ms == 0ul || config->settings.detect_period_ms == 0ul))
    {
        return operation_result_fail(ERROR_CODE_INVALID_ARGUMENT);
    }

    created_monitor = (alarm_monitor_t *)calloc(1u, sizeof(*created_monitor));
    ...
    if (!alarm_snapshot_mailbox_init(&created_monitor->snapshot_mailbox))
    {
...
operation_result_t alarm_monitor_start(alarm_monitor_t *monitor)
{
    operation_result_t result;
    ...
    worker_thread_config.entry = alarm_monitor_detect_entry;
    ...
    worker_thread_config.entry = alarm_monitor_io_entry;
    result = worker_thread_start(&monitor->io_worker_thread, &worker_thread_config);
    if (!result.ok)
    {
        alarm_monitor_stop(monitor);
        return result;
    }
    return operation_result_ok();
}

void alarm_monitor_stop(alarm_monitor_t *monitor)
{
    if (monitor == 0)
    {
        return;
...
void alarm_monitor_destroy(alarm_monitor_t *monitor)
{
    if (monitor == 0)
    {
        return;
