...
bool mqtt_command_parse(const char *json_str, cmd_t *out_cmd)
{
    ...
    action_item = cJSON_GetObjectItem(root, CMD_FIELD_ACTION);
    ...
    if (strcmp(action, CMD_ACTION_START_WASH) == 0)
    {
        out_cmd->type = CMD_START_WASH;
        /* 可选 mode 字段，默认 STANDARD */
        cJSON *mode_item = cJSON_GetObjectItem(root, CMD_FIELD_MODE);
    ...
    else if (strcmp(action, CMD_ACTION_STOP_WASH) == 0)
    {
        out_cmd->type = CMD_STOP_WASH;
        ok = true;
    }
    else if (strcmp(action, CMD_ACTION_STOP_OPERATION) == 0)
    {
        out_cmd->type = CMD_STOP_OPERATION;
        ok = true;
    }
    else if (strcmp(action, CMD_ACTION_RESUME_OPERATION) == 0)
    {
        out_cmd->type = CMD_RESUME_OPERATION;
        ok = true;
    }
    else if (strcmp(action, CMD_ACTION_RESET_FAULT) == 0)
    {
        out_cmd->type = CMD_RESET_FAULT;
        ok = true;
    }
    else if (strcmp(action, CMD_ACTION_HOME_DEVICE) == 0)
    {
