    ...
    unsigned long recorded_at_ms)
{
    ...
    snprintf(state_transition_record->record_id,
        sizeof(state_transition_record->record_id),
        "record-%lu",
        recorded_at_ms);
    ...
    if (previous_state != 0) {
        strncpy(state_transition_record->previous_state, previous_state, sizeof(state_transition_record->previous_state) - 1);
    }
    if (current_state != 0) {
        strncpy(state_transition_record->current_state, current_state, sizeof(state_transition_record->current_state) - 1);
    }
    if (result_code != 0) {
