    ...
    segment_control_evaluation_t *segment_control_evaluation)
{
    ...
    for (index = 0; index < wash_segment->conditional_control_count; ++index) {
        ...
        if (!active && position_trigger_matches(&conditional_control->start_trigger, &runtime_snapshot->position_snapshot)) {
            segment_control_evaluation->start_chemical[index] = true;
        }
        if (active && position_trigger_matches(&conditional_control->stop_trigger, &runtime_snapshot->position_snapshot)) {
            segment_control_evaluation->stop_chemical[index] = true;
        }
    }

    segment_control_evaluation->segment_complete = position_trigger_matches(
        &wash_segment->completion_condition.trigger,
        &runtime_snapshot->position_snapshot);
    return operation_result_ok();
}
