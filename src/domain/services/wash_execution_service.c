...
/**
 * @brief 初始化工步执行事实：记录生命周期转移、结果码、原因码、工步 ID
 * @param[out] wash_execution_fact 输出执行事实
 * @param wash_execution 当前执行状态
 * @param previous_state 转移前的生命周期状态
 * @param current_state 转移后的生命周期状态
 * @param result_code 结果码
 * @param reason_code 原因码
 */
static void init_fact(wash_execution_fact_t *wash_execution_fact, const wash_execution_t *wash_execution,
                      const char *previous_state, const char *current_state, const char *result_code,
...
/**
 * @brief 验证工步执行服务参数的完整性
 * @param wash_execution_service_args 执行服务参数
 * @param wash_execution_fact 输出事实缓冲区
 * @return true 表示任意必需参数为 null；false 表示全部合法
 * @details 检查执行态、会话、等待条件、快照、执行机构端口、传感器端口
 */
static bool invalid_args(wash_execution_service_args_t *wash_execution_service_args,
                         wash_execution_fact_t *wash_execution_fact)
{
    return wash_execution_service_args == 0 || wash_execution_service_args->wash_execution == 0 ||
           wash_execution_service_args->wash_session == 0 || wash_execution_service_args->wait_condition == 0 ||
           wash_execution_service_args->program_snapshot == 0 || wash_execution_service_args->actuator_port == 0 ||
           wash_execution_service_args->sensor_port == 0 || wash_execution_fact == 0;
}

/**
 * @brief 读取当前应执行的工步
 * @param wash_execution_service_args 执行服务参数
 * @return 指向当前工步的只读指针；索引越界或参数无效时返回 null
 * @details 基于 wash_execution->segment_index 查表；不修改状态
 */
static const wash_segment_t *current_segment(const wash_execution_service_args_t *wash_execution_service_args)
{
    ...
    return &wash_execution_service_args->program_snapshot->frozen_program
                .segments[wash_execution_service_args->wash_execution->segment_index];
}
                                                 ...
                                                 const wash_segment_t *wash_segment)
{
    ...
    if (actuator_port->set_roof_brush_follow != 0 &&
        actuator_port->set_roof_brush_follow(
            actuator_port->context, wash_segment->continuous_controls.roof_brush_follow, BRUSH_TIMEOUT_MS) != 0)
    {
        return operation_result_fail(ERROR_CODE_IO_FAILED);
    }
    if (actuator_port->set_side_brush_enabled != 0 &&
        actuator_port->set_side_brush_enabled(
            actuator_port->context, wash_segment->continuous_controls.side_brush_enabled, BRUSH_TIMEOUT_MS) != 0)
    {
        return operation_result_fail(ERROR_CODE_IO_FAILED);
    }
    if (actuator_port->set_ro_water_enabled != 0 &&
        actuator_port->set_ro_water_enabled(actuator_port->context, wash_segment->continuous_controls.ro_water_enabled,
                                            CLOSE_FEEDBACK_TIMEOUT_MS) != 0)
    {
        return operation_result_fail(ERROR_CODE_IO_FAILED);
    }
    if (actuator_port->set_dryer_enabled != 0 &&
        actuator_port->set_dryer_enabled(actuator_port->context, wash_segment->continuous_controls.dryer_enabled,
                                         CLOSE_FEEDBACK_TIMEOUT_MS) != 0)
    {
        return operation_result_fail(ERROR_CODE_IO_FAILED);
    }
...
static operation_result_t abort_with_recovery(wash_execution_service_args_t *wash_execution_service_args,
                                              ...
                                              const char *reason_code, wash_execution_fact_t *wash_execution_fact)
{
    ...
    wash_execution_abort(wash_execution_service_args->wash_execution, execution_result, end_reason,
                         wash_execution_service_args->current_time_ms);
    write_field(wash_execution_service_args->wash_execution->result_code,
                sizeof(wash_execution_service_args->wash_execution->result_code), result_code);
    ...
    init_fact(wash_execution_fact, wash_execution_service_args->wash_execution, previous_state, "aborted", result_code,
              reason_code);
    return operation_result_ok();
}
                                ...
                                wait_timeout_policy_t wait_timeout_policy)
{
    *wash_execution_service_args->next_wait_condition_sequence += 1;
    wait_condition_arm(wash_execution_service_args->wait_condition,
                       ...
                       wash_execution_service_args->current_time_ms,
                       wash_execution_service_args->current_time_ms + (unsigned long)timeout_ms, wait_timeout_policy,
                                     ...
                                     const wash_segment_t *wash_segment, wash_execution_fact_t *wash_execution_fact)
{
    ...
    wash_execution_begin_exit(wash_execution_service_args->wash_execution,
                              wash_execution_service_args->current_time_ms);
    ...
    init_fact(wash_execution_fact, wash_execution_service_args->wash_execution, "running", "exiting", "exiting",
              "segment_complete");
    return operation_result_ok();
}
                                              ...
                                              wash_execution_fact_t *wash_execution_fact)
{
    ...
    return abort_with_recovery(wash_execution_service_args, execution_result, EXECUTION_END_REASON_FAULT, "running",
                                  ...
                                  const runtime_snapshot_t *runtime_snapshot, unsigned long current_time_ms)
{
    ...
    if (wash_segment->exit_actions.stop_roof_brush)
    {
        wash_execution->exit_runtime.stop_roof_brush_done = feedback_or_fallback_done(
            ...
            runtime_snapshot->actuator_feedback.roof_brush_stopped, exit_elapsed_ms, CLOSE_FEEDBACK_TIMEOUT_MS);
    }
    if (wash_segment->exit_actions.stop_side_brush)
    {
        wash_execution->exit_runtime.stop_side_brush_done = feedback_or_fallback_done(
            ...
            runtime_snapshot->actuator_feedback.side_brush_stopped, exit_elapsed_ms, CLOSE_FEEDBACK_TIMEOUT_MS);
    }
    if (wash_segment->exit_actions.stop_chemical)
    {
        wash_execution->exit_runtime.stop_chemical_done = feedback_or_fallback_done(
            ...
            runtime_snapshot->actuator_feedback.chemical_closed, exit_elapsed_ms, CLOSE_FEEDBACK_TIMEOUT_MS);
    }
    if (wash_segment->exit_actions.close_ro_water)
    {
        wash_execution->exit_runtime.close_ro_water_done = feedback_or_fallback_done(
            ...
            runtime_snapshot->actuator_feedback.ro_water_closed, exit_elapsed_ms, CLOSE_FEEDBACK_TIMEOUT_MS);
    }
    if (wash_segment->exit_actions.close_dryer)
    {
        wash_execution->exit_runtime.close_dryer_done = feedback_or_fallback_done(
            ...
            runtime_snapshot->actuator_feedback.dryer_closed, exit_elapsed_ms, CLOSE_FEEDBACK_TIMEOUT_MS);
    }
                                                             ...
                                                             wash_execution_fact_t *wash_execution_fact)
{
    ...
    wash_segment = &wash_execution_service_args->program_snapshot->frozen_program
                        .segments[wash_execution_service_args->wash_execution->segment_index + 1];
    *wash_execution_service_args->next_execution_sequence += 1;
    wash_execution_start_segment(
        wash_execution_service_args->wash_execution, wash_execution_service_args->wash_session->session_id,
        wash_execution_service_args->wash_execution->segment_index + 1, wash_segment->segment_id,
        wash_execution_service_args->current_time_ms, *wash_execution_service_args->next_execution_sequence);
    wash_session_bind_execution(wash_execution_service_args->wash_session,
                                wash_execution_service_args->wash_execution->execution_id, wash_segment->segment_id);
    init_fact(wash_execution_fact, wash_execution_service_args->wash_execution, "pending", "entering", "entering",
              "segment_loaded");
    return operation_result_ok();
}
                                               ...
                                               wash_execution_fact_t *wash_execution_fact)
{
    ...
    if (wash_execution_service_args->wash_execution->lifecycle_state == SEGMENT_LIFECYCLE_ENTERING)
    {
        ...
        init_fact(wash_execution_fact, wash_execution_service_args->wash_execution, "entering", "running", "running",
                  "segment_running");
        return operation_result_ok();
    }
    ...
    {
        result = segment_control_service_evaluate(wash_segment, wash_execution_service_args->wash_execution,
                                                  &runtime_snapshot, &segment_control_evaluation);
        if (!result.ok)
        {
            return result;
        }
        ...
        {
            return handle_sensor_fault(wash_execution_service_args, wash_segment,
                                       wash_segment->exception_policy.on_position_lost, EXECUTION_RESULT_POSITION_LOST,
                                       "position_lost", wash_execution_fact);
        }
        if (segment_control_evaluation.follow_lost)
        {
            return handle_sensor_fault(wash_execution_service_args, wash_segment,
                                       wash_segment->exception_policy.on_follow_lost, EXECUTION_RESULT_FOLLOW_LOST,
                                       "follow_lost", wash_execution_fact);
        }
        ...
        for (index = 0; index < wash_segment->conditional_control_count; ++index)
        {
                ...
                wash_execution_service_args->actuator_port->set_chemical_enabled != 0)
            {
                if (wash_execution_service_args->actuator_port->set_chemical_enabled(
                        wash_execution_service_args->actuator_port->context, true, CHEMICAL_TIMEOUT_MS) != 0)
                {
                    return operation_result_fail(ERROR_CODE_IO_FAILED);
                }
            ...
            {
                if (wash_execution_service_args->actuator_port->set_chemical_enabled(
                        wash_execution_service_args->actuator_port->context, false, CHEMICAL_TIMEOUT_MS) != 0)
                {
                    return operation_result_fail(ERROR_CODE_IO_FAILED);
                }
                wash_execution_service_args->wash_execution->active_conditional_controls[index] = false;
    ...
    {
        if (all_exit_actions_done(wash_segment, wash_execution_service_args->wash_execution, &runtime_snapshot,
                                  wash_execution_service_args->current_time_ms))
        {
            wait_condition_mark_done(wash_execution_service_args->wait_condition);
            wash_execution_complete(wash_execution_service_args->wash_execution, EXECUTION_RESULT_SEGMENT_COMPLETED,
                                    EXECUTION_END_REASON_NORMAL, wash_execution_service_args->current_time_ms);
            write_field(wash_execution_service_args->wash_execution->result_code,
                        sizeof(wash_execution_service_args->wash_execution->result_code), "completed");
            write_field(wash_execution_service_args->wash_execution->reason_code,
                        sizeof(wash_execution_service_args->wash_execution->reason_code), "exit_complete");
            init_fact(wash_execution_fact, wash_execution_service_args->wash_execution, "exiting", "completed",
                      "completed", "exit_complete");
        }
    }
    return operation_result_ok();
}
                                                      ...
                                                      wash_execution_fact_t *wash_execution_fact)
{
    ...
    wait_condition_mark_done(wash_execution_service_args->wait_condition);
    wash_execution_abort(wash_execution_service_args->wash_execution, EXECUTION_RESULT_STOPPED,
                         EXECUTION_END_REASON_STOP, wash_execution_service_args->current_time_ms);
    write_field(wash_execution_service_args->wash_execution->result_code,
                sizeof(wash_execution_service_args->wash_execution->result_code), "stopped");
    write_field(wash_execution_service_args->wash_execution->reason_code,
                sizeof(wash_execution_service_args->wash_execution->reason_code), reason_code);
                                                       ...
                                                       wash_execution_fact_t *wash_execution_fact)
{
    ...
    wait_condition_mark_done(wash_execution_service_args->wait_condition);
    wash_execution_abort(wash_execution_service_args->wash_execution, EXECUTION_RESULT_FAULTED,
                         EXECUTION_END_REASON_FAULT, wash_execution_service_args->current_time_ms);
    write_field(wash_execution_service_args->wash_execution->result_code,
                sizeof(wash_execution_service_args->wash_execution->result_code), "faulted");
    write_field(wash_execution_service_args->wash_execution->reason_code,
                sizeof(wash_execution_service_args->wash_execution->reason_code), reason_code);
    init_fact(wash_execution_fact, wash_execution_service_args->wash_execution,
                                                         ...
                                                         wash_execution_fact_t *wash_execution_fact)
{
    ...
    exception_strategy = currently_exiting ? wash_segment->exception_policy.on_exit_timeout
    ...
    wash_execution_abort(wash_execution_service_args->wash_execution,
                         ...
                         EXECUTION_END_REASON_TIMEOUT, wash_execution_service_args->current_time_ms);
    write_field(wash_execution_service_args->wash_execution->result_code,
    ...
    write_field(wash_execution_service_args->wash_execution->reason_code,
                ...
                currently_exiting ? "exit_timeout" : "segment_timeout");
    init_fact(wash_execution_fact, wash_execution_service_args->wash_execution,
              currently_exiting ? "exiting" : "running", "aborted",
              currently_exiting ? "exit_timeout" : "segment_timeout",
              currently_exiting ? "exit_timeout" : "segment_timeout");
    return operation_result_ok();
}
