...
static bool engine_resolve(void *ctx, const char *name, double *out)
{
    ...
    if (strncmp(name, "axes.", 5) == 0)
    {
        ...
        if (idlen >= sizeof(id)) { return false; }
        ...
        if (strcmp(field, "position") == 0) { *out = pos; return true; }
        if (strcmp(field, "speed") == 0)    { *out = speed; return true; }
        if (strcmp(field, "valid") == 0)    { *out = valid ? 1.0 : 0.0; return true; }
        return false;
    }
    ...
    if (strncmp(name, "markers.", 8) == 0)
    {
        ...
        {
            if ((strncmp(e->prog->markers[i].id, rest, idlen) == 0) &&
                (e->prog->markers[i].id[idlen] == '\0'))
            {
                const char *field = dot + 1;
                if (strcmp(field, "position") == 0) { *out = e->markers[i].position; return true; }
                if (strcmp(field, "valid") == 0)    { *out = e->markers[i].valid ? 1.0 : 0.0; return true; }
                return false;
            }
        }
        return false;
...
static bool edge_hit(engine_edge_t edge, int prev, int cur)
{
    switch (edge)
    {
        case ENGINE_EDGE_RISING:  return (prev == 0) && (cur != 0);
        case ENGINE_EDGE_FALLING: return (prev != 0) && (cur == 0);
        case ENGINE_EDGE_HIGH:    return (cur != 0);
        case ENGINE_EDGE_LOW:     return (cur == 0);
...
static bool trigger_level_active(engine_t *e, const engine_trigger_t *trig)
{
    ...
    switch (trig->edge)
    {
        ...
        case ENGINE_EDGE_LOW:  return (cur == 0);
        default:               return (cur != 0); /* rising/high */
    }
}
...
static bool enter_phase(engine_t *e, int idx)
{
    ...
    for (unsigned i = 0U; i < ph->lane_count; ++i)
    {
        ...
        if (e->lanes[i].steps == NULL) { return false; }
        e->lanes[i].count = sc;
        for (unsigned j = 0U; j < sc; ++j)
        {
            ...
            rt->done_elapsed = 0U;
            rt->trig_prev    = (sd->trigger.type == ENGINE_TRIG_SIGNAL)
                                   ? sig_read(sd->trigger.signal) : 0;
        }
    }
    return true;
}
            ...
            case RT_WAIT_DONE:
            {
                switch (sd->done.type)
                {
                    ...
                    case ENGINE_DONE_SIGNAL:
                    {
                        ...
                        {
                            rt->done_elapsed += dt;
                            if ((sd->done.timeout_ms > 0U) &&
                                (rt->done_elapsed >= sd->done.timeout_ms))
                            {
...
static void phase_tick(engine_t *e, uint32_t dt)
{
    ...
    {
        if (ph->on_timeout == ENGINE_ERR_STOP) { do_halt_all(e); }
        else                                   { do_halt_phase(e); }
        return;
    }
...
static void collect_do_channel(engine_t *e, const char *name)
{
    ...
    for (unsigned i = 0U; i < e->do_count; ++i)
    {
        if (strcmp(e->do_channels[i], name) == 0) { return; }
    }
...
static bool build_runtime_tables(engine_t *e)
{
    ...
    if (p->marker_count > 0U)
    {
        ...
        if (e->markers == NULL) { return false; }
    }
    if (p->interlock_count > 0U)
    {
        ...
        if ((e->ilk_active == NULL) || (e->ilk_order == NULL)) { return false; }
        for (unsigned i = 0U; i < p->interlock_count; ++i) { e->ilk_order[i] = (int)i; }
    ...
    if (e->do_channels == NULL) { return false; }
    e->do_count = 0U;
...
{
    free(e->markers);    e->markers = NULL;
    free(e->ilk_active); e->ilk_active = NULL;
    free(e->ilk_order);  e->ilk_order = NULL;
    free(e->do_channels); e->do_channels = NULL;
    e->do_count = 0U;
}

sw_err_t engine_load_program(engine_t *e, engine_program_t *prog)
{
    if ((e == NULL) || (prog == NULL)) { return SW_ERR_PARAM; }
...
{
    if ((e == NULL) || (e->prog == NULL)) { return SW_ERR_STATE; }
    if (e->prog->phase_count == 0U) { return SW_ERR_STATE; }
...
{
    if ((e == NULL) || (e->state == ENGINE_STATE_IDLE)) { return -1; }
    return e->cur_phase;
}

const char *engine_current_phase_id(const engine_t *e)
{
    if ((e == NULL) || (e->cur_def == NULL)) { return NULL; }
    return e->cur_def->id;
}
