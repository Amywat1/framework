 ...
 *          后者在进入 IDLE 时发布 motion_completed（轴已空闲）。
 * @note    回原不单独提供 API：对 ORIGIN 限位下发 run(..., spec)，
 *          触原点后的基准重建由电机执行器完成。
 * @note    位置/方向/故障码等查询直接使用 hal_motor_*，本模式不透传。
 */
 ...
 * @note   完成须 poll state == IDLE，或依赖 motor_axis_poll() 发完成事件
...
/**
 * @brief  推进完成边沿：若存在未完成意图且已进入 IDLE，则发布 motion_completed
 * @param[in,out] self 模式实例
 * @note   有完成事件需求的轴须由调用方周期调用；actuator_id 为 0 时不发布
 */
