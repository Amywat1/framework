...
| 事件                   | 携带数据                                     | 触发条件                                                        |
| -------------------- | ---------------------------------------- | ----------------------------------------------------------- |
| WashSessionStarted   | session_id, program_id                   | CREATED → RUNNING                                           |
| PassStarted          | session_id, pass_index, direction        | 每趟开始                                                        |
| PassCompleted        | session_id, pass_index                   | 每趟完成                                                        |
...
> RecoveryCompleted 例外：由 `recovery_service.c`（应用层）在恢复流程结束后直接发布。

| 事件                     | 携带数据                               | 触发条件      |
| ---------------------- | ---------------------------------- | --------- |
| OperationalModeChanged | from_mode, to_mode                 | 模式切换      |
| CommandRejected        | command_type, reason, current_mode | 指令被拒绝     |
| RecoveryRequested      | —                                  | RecoverCommand 合法，进入 RECOVERING；应用服务订阅后协调 AlarmRegistry + DeviceControl |
| RecoveryCompleted      | result: IDLE \| EXCEPTION          | RECOVERING 结束（发布者：recovery_service，非 OperationalMode） |
...
```
  ...
  │    │  B. EStop 触发（任意模式）                           │        │ │
  │    │  C. PostWashAssessmentCompleted(enter_exception=true)│        │ │
  │    └──────────────────┬───────────────────────────────────┘        │ │
  │                       ▼                                            │ │
  │              ┌──────────────────┐                                  │ │
  │              │    EXCEPTION     │                                  │ │
...
转换规则：
  ...
  RUNNING    + PassCompleted（非最后趟）     → RUNNING（PassExecutor 重置，启动下一趟）
  RUNNING    + PassCompleted（最后一趟）     → COMPLETING
...
| 事件                   | 触发时机                   | 携带数据                                     |
| -------------------- | ---------------------- | ---------------------------------------- |
| WashSessionStarted   | CREATED → RUNNING      | session_id, program_id                   |
| PassStarted          | 每趟开始                   | session_id, pass_index                   |
| PassCompleted        | 每趟完成                   | session_id, pass_index                   |
...
校验流程：
  ...
  4. DENIED 时发布 CommandRejected 事件
```
...
【指令触发】（外部指令，经过合法性校验后执行）
  ...
  StartWashCommand（在 IDLE）     → 转交洗车执行，等待 WashSessionStarted 确认
  StartSelfCheckCommand（在 IDLE）→ IDLE → SELF_CHECK
  RecoverCommand（在 MANUAL/EXCEPTION，EStop 未激活）
                                  → MANUAL/EXCEPTION → RECOVERING
                                  → 发布 RecoveryRequested 事件
                                  （跨聚合协调由应用服务完成：
                                    alarm_registry_recover_all() + device 驱动机构到安全位置）

【事件触发】（订阅其他上下文的领域事件）
  WashSessionStarted              → IDLE → WASHING
  WashSessionCompleted            → WASHING → IDLE（由 PostWash 决定最终落点）
  WashSessionAborted(CRITICAL_ALARM / STEP_TIMEOUT)
                                  → WASHING → EXCEPTION
  WashSessionAborted(MANUAL)      → WASHING → IDLE
  PostWashAssessmentCompleted(enter_exception=true)
                                  → IDLE → EXCEPTION
  SelfCheckCompleted              → SELF_CHECK → IDLE
  AlarmTriggered(CRITICAL)        → IDLE / MANUAL / SELF_CHECK → 直接进入 EXCEPTION
                                  → WASHING    → 忽略（WashSession 先行处理 →
                                                   WashSessionAborted(CRITICAL_ALARM) 路径）
                                  → EXCEPTION  → 已在异常态，幂等保持
                                  → RECOVERING → 不中断恢复流程（BR-20 禁止期间响应外部指令）；
                                                   recovery_service 在发出 RecoveryCompleted 前
                                                   检查 alarm_registry 是否存在活跃 CRITICAL 报警，
                                                   有则强制 result = EXCEPTION
  EStopTriggered                  → 任意 → EXCEPTION
                                  （若已在 EXCEPTION/RECOVERING 则保持，更新 estop_active）
  AlarmCleared(EStop)             → 更新 estop_active = false（不改变模式）
  RecoveryCompleted(IDLE)         → RECOVERING → IDLE
  RecoveryCompleted(EXCEPTION)    → RECOVERING → EXCEPTION
```
...
/* 事件输入（来自事件总线订阅）*/
op_mode_on_wash_session_started(mode)
...
op_mode_on_recovery_completed(mode, result)     ← result: IDLE | EXCEPTION
...
| CommandRejected        | 指令被拒绝                                         | command_type, reason, current_mode |
| RecoveryRequested      | RecoverCommand 合法，进入 RECOVERING               | —（应用服务订阅后协调 AlarmRegistry + DeviceControl） |

> 注：RecoveryCompleted 由 `recovery_service.c`（应用层）发布，不在 OperationalMode 发布事件列表中。
...
- 发布 AlarmTriggered / AlarmCleared / PostWashAssessmentCompleted 事件
...
```
触发：应用层订阅 WashSessionCompleted 或 WashSessionAborted 后，
...
下游响应：
  应用层订阅此事件，调用 op_mode_on_post_wash_assessment(enter_exception)：
...
alarm_registry_on_wash_session_ended(registry, session_event)
  ← 触发 PostWashAssessment
...
alarm_registry_is_estop_active(registry)        → bool
alarm_registry_get_active_warnings(registry, buf, max) → count
...
| PostWashAssessmentCompleted | 服务完成评估结束        | blocking_alarms[], enter_exception   |
...
│   ├── infrastructure/
│   │   ├── device/IGantryPort
│   │   │   ├── gantry_can.h/.c      ← IGantryPort 的 CAN 实现
│   │   │   ├── actuator_can.h/.c    ← IActuatorPort 的 CAN/GPIO 实现
...
```c
/* === infrastructure/device/gantry_can.c（CAN 适配器实现）=== */

static void can_gantry_move_to(void *ctx, int32_t target_mm, int32_t speed_mm_s) {
    CanFrame f = build_gantry_move_frame(target_mm, speed_mm_s);
    can_driver_send(&f);
}

static void can_gantry_stop(void *ctx) {
    CanFrame f = build_gantry_stop_frame();
    can_driver_send(&f);
}

static int32_t can_gantry_get_position(void *ctx) {
    return g_gantry_pos_mm;  /* 由 CAN 接收中断持续更新 */
}

/* 导出静态适配器实例（供 main.c 注入）*/
const IGantryPort g_gantry_can_port = {
    .move_to         = can_gantry_move_to,
    .stop            = can_gantry_stop,
    .get_position_mm = can_gantry_get_position,
};
```

```c
/* === main.c / app_init.c：依赖注入 =================================================*/

static WashSession     g_wash_session;
static OperationalMode g_op_mode;
static AlarmRegistry   g_alarm_registry;

void app_init(void) {
    /* 注入端口（将 CAN 适配器绑定到领域聚合）*/
    WashSessionPorts ports = {
        .gantry   = &g_gantry_can_port,
        .actuator = &g_actuator_can_port,
        .chemical = &g_chemical_gpio_port,
    };
    wash_session_init(&g_wash_session, &ports);
    op_mode_init(&g_op_mode);
    alarm_registry_init(&g_alarm_registry, alarm_definitions, ALARM_DEF_COUNT);
    ...
    /* 洗车会话启动 → 模式 IDLE→WASHING */
    event_bus_subscribe(EVT_WASH_SESSION_STARTED,         cmd_handler_on_wash_session_started);
      /* cmd_handler_on_wash_session_started 内部：
           op_mode_on_wash_session_started(&g_op_mode) */

    /* 洗车会话结束 → 触发 PostWashAssessment + 模式转换 */
    ...
    event_bus_subscribe(EVT_WASH_SESSION_ABORTED,         cmd_handler_on_wash_session_ended);
      /* cmd_handler_on_wash_session_ended 内部：
           alarm_registry_on_wash_session_ended(&g_alarm_registry, evt)  （执行评估）
           op_mode_on_wash_session_completed/aborted(&g_op_mode, evt)    （模式转换） */

    /* PostWashAssessment 结果 → 应用层喂给 OperationalMode */
    event_bus_subscribe(EVT_POST_WASH_ASSESSMENT_DONE,    cmd_handler_on_post_wash_assessment);
      /* cmd_handler_on_post_wash_assessment 内部：
           op_mode_on_post_wash_assessment(&g_op_mode, evt.enter_exception) */
    ...
    /* RecoveryRequested → recovery_service 协调跨聚合动作 */
    event_bus_subscribe(EVT_RECOVERY_REQUESTED,           recovery_service_on_requested);
      /* recovery_service_on_requested 内部：
           alarm_registry_recover_all(&g_alarm_registry)     （清除所有可清除报警）
           device_drive_to_safe_state()                       （并行驱动机构到安全位置）
           → 等待机构到位或超时
           → 检查 alarm_registry 是否存在活跃 CRITICAL 报警
               （恢复过程中可能新触发，如 CAN 通信故障）
           result = IDLE     若机构全部到位 且 无活跃 CRITICAL 报警
           result = EXCEPTION 若超时 或 有活跃 CRITICAL 报警
           → 发布 RecoveryCompleted { result }               （recovery_service 自己发布）*/

    /* RecoveryCompleted → 模式 RECOVERING→IDLE/EXCEPTION */
    event_bus_subscribe(EVT_RECOVERY_COMPLETED,           cmd_handler_on_recovery_completed);
      /* cmd_handler_on_recovery_completed 内部：
           op_mode_on_recovery_completed(&g_op_mode, evt.result) */
 ...
 * （单趟可能产生多个 PassStarted / ActuatorStateChanged，超过 buf 大小时需多轮拉取）
 */
