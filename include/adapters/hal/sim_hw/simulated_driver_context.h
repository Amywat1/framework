#ifndef ADAPTERS_HAL_SIM_HW_SIMULATED_DRIVER_CONTEXT_H
#define ADAPTERS_HAL_SIM_HW_SIMULATED_DRIVER_CONTEXT_H
...
typedef struct simulated_driver_context_t
{
    ...
    /* 启动预检仿真字段：默认全通过，置反可覆盖触发对应拒绝路径 */
    bool precheck_read_should_fail;    /**< 为 true 时 read_snapshot 返回 -1（独立于 read_machine_snapshot） */
    bool precheck_safety_should_fail;  /**< 为 true 可触发安全互锁拒绝 */
    bool precheck_estop_active;        /**< 为 true 可触发急停拒绝 */
    bool precheck_vehicle_absent;      /**< 为 true 可触发车辆未检测到拒绝 */
    bool precheck_vehicle_not_allowed; /**< 为 true 可触发车型不允许拒绝 */
    bool precheck_resource_fail;       /**< 为 true 可触发资源未就绪拒绝 */
    bool precheck_position_fail;       /**< 为 true 可触发位置检查失败拒绝 */
