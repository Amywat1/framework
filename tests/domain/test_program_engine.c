...
static void test_engine_runtime_waits_for_motion_settlement(void)
{
    engine_t         *engine = engine_create(&s_environment);
    engine_program_t *prog   = make_program();
    engine_step_t    *start  = &prog->phases[0].lanes[0].steps[0];
    engine_step_t    *next   = &prog->phases[0].lanes[0].steps[1];
    unsigned          resource_id;

    start->done.type = ENGINE_DONE_MOTION;
    (void)snprintf(start->done.resource, sizeof(start->done.resource), "%s", "aout");
    start->done.timeout_ms = 1000U;
    next->trigger.type     = ENGINE_TRIG_CONDITION;
    next->trigger.cond     = compile_ok("true");

    TEST_ASSERT_NOT_NULL(engine);
    TEST_ASSERT_EQUAL_INT(SW_OK, engine_load_program(engine, prog));
    resource_id = start->done.resource_id;
    TEST_ASSERT_EQUAL_INT(SW_OK, engine_start(engine));
    ...
    TEST_ASSERT_EQUAL_INT(SW_OK, engine_actuator_release(s_environment.actuator, resource_id));
...
int main(void)
{
    ...
    WDF_RUN_TEST(test_engine_runtime_waits_for_motion_settlement, "", "验证程序引擎等待机构运动结算");
