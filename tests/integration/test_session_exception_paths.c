...
static int verify_timeout_path(void)
{
    ...
    result = test_start_session_and_flush( "standard_wash");
    TEST_ASSERT(result.ok);
    control_context_private_wait_condition_mutable()->timeout_policy = WAIT_TIMEOUT_POLICY_SEGMENT;
    ...
    TEST_ASSERT(result.ok);
    TEST_ASSERT(control_context_private_wash_session()->session_state == SESSION_STATE_ABORTED);
    TEST_ASSERT(control_context_private_wash_session()->final_session_result == RESULT_CODE_SEGMENT_TIMEOUT);
    TEST_ASSERT(control_context_private_wash_execution()->execution_result == EXECUTION_RESULT_SEGMENT_TIMEOUT);
    TEST_ASSERT(strcmp(control_context_last_result_code(), "aborted") == 0);
    TEST_ASSERT(strcmp(control_context_last_reason_code(), "segment_timeout") == 0);
    TEST_ASSERT(control_context_private_device_state() == DEVICE_STATE_EXCEPTION);
    return 0;
}
...
static int verify_idle_fault_clear_path(void)
{
    ...
    TEST_ASSERT(strcmp(control_context_private_last_transition_record()->reason_code, "global_fault_recorded") == 0);
