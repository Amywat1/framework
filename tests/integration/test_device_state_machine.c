...
static int homing_alarm_injection_stop_all(void *context, int timeout_ms)
{
    ...
    result = homing_alarm_injection_context->actuator_port.stop_all(
        ...
        timeout_ms);
    if (result == 0) {
        device_runtime_private_set_global_fault(homing_alarm_injection_context->system_context,
            "RECOVERING_FAULT",
            "recovering-alarm");
    }
    return result;
...
static int verify_power_on_defaults_to_stopped_and_blocks_start(void)
{
    ...
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_reason_code, "device_state_stopped") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.result_code, "rejected") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.reason_code, "device_state_stopped") == 0);
...
static int verify_homing_rejected_from_idle(void)
{
    ...
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_result_code, "rejected") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_reason_code, "homing_requires_stopped") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.result_code, "rejected") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.reason_code, "homing_requires_stopped") == 0);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->last_transition_record.trigger_type == TRIGGER_TYPE_HOMING);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->device_state == DEVICE_STATE_IDLE);
...
static int verify_homing_clears_alarm_before_recovering(void)
{
    ...
    test_setup_system_context(&system_context, &driver_context);
    device_runtime_private_set_global_fault(system_context, "STALE_FAULT", "stale-alarm");
    ...
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.reason_code, "homing_completed") == 0);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->last_transition_record.trigger_type == TRIGGER_TYPE_HOMING);
...
static int verify_homing_success_runs_real_flow_and_returns_to_idle(void)
{
    ...
    TEST_ASSERT(result.ok);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->device_state == DEVICE_STATE_RUNNING);

    final_segment_index = device_runtime_private_runtime(system_context)->program_snapshot.frozen_program.segment_count - 1;
    device_runtime_private_runtime(system_context)->wash_execution.segment_index = final_segment_index;
    device_runtime_private_runtime(system_context)->wash_execution.execution_state = EXECUTION_STATE_COMPLETED;
    device_runtime_private_runtime(system_context)->wash_execution.state = SEGMENT_LIFECYCLE_COMPLETED;
...
static int verify_homing_alarm_during_recovering_moves_device_to_exception(void)
{
    ...
    homing_alarm_injection_context.actuator_port = device_runtime_private_runtime(system_context)->actuator_port;
    homing_alarm_injection_context.sensor_port = device_runtime_private_runtime(system_context)->sensor_port;
    ...
    device_runtime_set_actuator_port(system_context, &actuator_port);
    ...
    device_runtime_set_sensor_port(system_context, &sensor_port);
...
static int verify_trigger_start_rejected_from_exception_with_matrix_reason(void)
{
    ...
    TEST_ASSERT(result.error_code == ERROR_CODE_INVALID_STATE);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_result_code, "rejected") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_reason_code, "device_state_exception") == 0);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->last_transition_record.trigger_type == TRIGGER_TYPE_START);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.result_code, "rejected") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.reason_code, "device_state_exception") == 0);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->device_state == DEVICE_STATE_EXCEPTION);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->global_fault_present);
...
static int verify_trigger_stop_rejected_from_stopped_and_idle(void)
{
    ...
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.result_code, "rejected") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.reason_code, "stop_requires_running") == 0);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->device_state == DEVICE_STATE_STOPPED);
...
static int verify_trigger_fault_clear_rejected_from_stopped_and_idle(void)
{
    ...
    TEST_ASSERT(device_runtime_private_runtime(system_context)->last_transition_record.trigger_type == TRIGGER_TYPE_FAULT);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.result_code, "rejected") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.reason_code, "fault_clear_requires_exception") == 0);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->device_state == DEVICE_STATE_STOPPED);
...
static int verify_trigger_homing_accepted_from_stopped(void)
{
    ...
    TEST_ASSERT(device_runtime_private_runtime(system_context)->last_transition_record.trigger_type == TRIGGER_TYPE_HOMING);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.previous_state, "stopped") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.current_state, "idle") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.result_code, "accepted") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.reason_code, "homing_completed") == 0);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->device_state == DEVICE_STATE_IDLE);
...
static int verify_trigger_start_accepted_from_idle(void)
{
    ...
    TEST_ASSERT(device_runtime_private_runtime(system_context)->device_state == DEVICE_STATE_RUNNING);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_result_code, "accepted") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_reason_code, "session_started") == 0);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->last_transition_record.trigger_type == TRIGGER_TYPE_START);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.previous_state, "idle") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.current_state, "running") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.result_code, "accepted") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.reason_code, "session_started") == 0);
...
static int verify_trigger_stop_accepted_from_running(void)
{
    ...
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_reason_code, "trigger-stop-accepted") == 0);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->last_transition_record.trigger_type == TRIGGER_TYPE_STOP);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.previous_state, "running") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.current_state, "stopped") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.result_code, "aborted") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.reason_code, "trigger-stop-accepted") == 0);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->device_state == DEVICE_STATE_STOPPED);
...
static int verify_trigger_fault_clear_accepted_from_exception(void)
{
    ...
    TEST_ASSERT(device_runtime_private_runtime(system_context)->last_transition_record.trigger_type == TRIGGER_TYPE_FAULT);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.previous_state, "exception") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.current_state, "stopped") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.result_code, "accepted") == 0);
