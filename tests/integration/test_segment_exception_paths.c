...
static int test_exit_timeout_aborts_session(void)
{
    ...
    TEST_ASSERT(result.ok);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->wash_session.session_state == SESSION_STATE_ABORTED);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->wash_session.final_session_result == RESULT_CODE_EXIT_FAILED);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->wash_execution.execution_result == EXECUTION_RESULT_EXIT_TIMEOUT);
    ...
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_reason_code, "exit_timeout") == 0);
...
static int test_runtime_snapshot_read_failure_enters_safe_stop(void)
{
    ...
    TEST_ASSERT(device_runtime_private_runtime(system_context)->wash_execution.execution_state == EXECUTION_STATE_ABORTED);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->wash_session.session_state == SESSION_STATE_ABORTED);
