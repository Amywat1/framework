...
static int recovering_probe_stop_all(void *context, int timeout_ms)
{
    ...
    recovering_probe_context->stop_all_observed_recovering =
        control_context_private_runtime_mutable()->device_state == DEVICE_STATE_RECOVERING;
...
static void install_recovering_probe(recovering_probe_context_t *recovering_probe_context)
{
    ...
    recovering_probe_context->actuator_port = control_context_private_runtime_mutable()->actuator_port;
...
static int verify_manual_stop_cycle_path(void)
{
    ...
    TEST_ASSERT(strcmp(control_context_private_runtime_mutable()->last_reason_code, "manual-stop") == 0);
