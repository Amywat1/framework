...
int main(void)
{
    ...
    updated_program = device_runtime_private_runtime(system_context)->program_snapshot.frozen_program;
    updated_program.revision = 9;
    file_program_repository_set_runtime_program(system_context, &updated_program, 9);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->program_snapshot.source_revision != 9);
