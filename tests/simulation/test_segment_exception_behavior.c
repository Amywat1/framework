...
static int test_ro_water_close_fallback(void)
{
    ...
    TEST_ASSERT(result.ok);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->wash_execution.segment_id, "dryer_segment") == 0);
...
static int test_chemical_command_failure_does_not_flip_rule_state(void)
{
    ...
    TEST_ASSERT(!result.ok);
    TEST_ASSERT(result.error_code == ERROR_CODE_IO_FAILED);
    TEST_ASSERT(!device_runtime_private_runtime(system_context)->wash_execution.active_conditional_controls[0]);
...
static int test_exit_command_failure_does_not_half_switch_state(void)
{
    ...
    TEST_ASSERT(result.error_code == ERROR_CODE_IO_FAILED);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->wash_execution.state == SEGMENT_LIFECYCLE_RUNNING);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->wait_condition.timeout_policy != WAIT_TIMEOUT_POLICY_EXIT);
    test_release_system_context(system_context);
    return 0;
}
