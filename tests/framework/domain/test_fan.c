 ...
 *          各测试自行在函数体内调用 fan_init(s_hexec, 0) 以进入已初始化状态。
...
{
    TEST_ASSERT_EQUAL(SW_ERR_NOT_INIT, fan_recover(HAL_MOTOR_RECOVERY_MODULE_STOP));
}
...
{
    TEST_ASSERT_EQUAL(SW_OK, fan_init(s_hexec, 0));
}
...
void test_fan_recover_module_stop_clears_fault(void)
{
    fan_init(s_hexec, 0);
    s_exec.m[0].phase = MOTOR_PHASE_FAULT;
    TEST_ASSERT_EQUAL(SW_OK, fan_recover(HAL_MOTOR_RECOVERY_MODULE_STOP));
...
void test_fan_recover_driver_reset_returns_ok(void)
{
    fan_init(s_hexec, 0);
    s_exec.m[0].phase = MOTOR_PHASE_FAULT;
    TEST_ASSERT_EQUAL(SW_OK, fan_recover(HAL_MOTOR_RECOVERY_DRIVER_RESET));
}
...
void test_fan_recover_then_start_ok(void)
{
    fan_init(s_hexec, 0);
    s_exec.m[0].phase = MOTOR_PHASE_FAULT;
    fan_recover(HAL_MOTOR_RECOVERY_MODULE_STOP);
    TEST_ASSERT_EQUAL(SW_OK, fan_start());
    TEST_ASSERT_EQUAL(FAN_STATE_RUNNING, fan_state());
}
