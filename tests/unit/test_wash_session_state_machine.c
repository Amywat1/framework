...
static int verify_start_returns_transition_fact(void)
{
    ...
    control_context_advance_time(100ul);
    program_snapshot_service_args = test_build_program_snapshot_service_args();
    result = program_snapshot_service_capture(&program_snapshot_service_args, "standard_wash");
    TEST_ASSERT(result.ok);

    wash_session_service_args = test_build_wash_session_service_args();
...
static int verify_complete_returns_transition_fact(void)
{
    ...
    wash_session_service_args.current_time_ms = 200ul;
...
static int verify_abort_returns_transition_fact(void)
{
    ...
    wash_session_service_args.current_time_ms = 300ul;
