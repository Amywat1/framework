 ...
 *          各测试自行在函数体内调用 lift_init(&s_exec, 0) 以进入已初始化状态。
...
{
    TEST_ASSERT_EQUAL(SW_ERR_NOT_INIT, lift_recover(MOTOR_RECOVERY_MODULE_STOP));
}
...
{
    TEST_ASSERT_EQUAL(SW_OK, lift_init(&s_exec, 0));
}
...
void test_lift_recover_module_stop_clears_fault(void)
{
    lift_init(&s_exec, 0);
    s_exec.m[0].phase = MOTOR_PHASE_FAULT;
    TEST_ASSERT_EQUAL(SW_OK, lift_recover(MOTOR_RECOVERY_MODULE_STOP));
...
void test_lift_recover_driver_reset_returns_ok(void)
{
    lift_init(&s_exec, 0);
    s_exec.m[0].phase = MOTOR_PHASE_FAULT;
    TEST_ASSERT_EQUAL(SW_OK, lift_recover(MOTOR_RECOVERY_DRIVER_RESET));
}
...
void test_lift_recover_then_up_ok(void)
{
    lift_init(&s_exec, 0);
    s_exec.m[0].phase = MOTOR_PHASE_FAULT;
    lift_recover(MOTOR_RECOVERY_MODULE_STOP);
    TEST_ASSERT_EQUAL(SW_OK, lift_up(0, NULL));
    TEST_ASSERT_EQUAL(LIFT_STATE_LIFTING, lift_state());
}
