...
int main(void)
{
    ...
    strncpy(snapshot_id, device_runtime_private_runtime(system_context)->program_snapshot.program_snapshot_id, sizeof(snapshot_id) - 1);
    ...
    TEST_ASSERT(result.ok);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->program_snapshot.program_snapshot_id[0] == '\0');
    TEST_ASSERT(device_runtime_private_runtime(system_context)->program_snapshot.validation_result
    ...
    result = test_start_session_and_flush(system_context, "standard_wash");
    TEST_ASSERT(result.ok);
    TEST_ASSERT(strcmp(snapshot_id, device_runtime_private_runtime(system_context)->program_snapshot.program_snapshot_id) == 0);
