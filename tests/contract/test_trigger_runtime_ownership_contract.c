...
static int verify_main_loop_owns_queue_and_time(void)
{
    ...
    control_tick_advance_time(system_context, 25ul);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->current_time_ms == 25ul);
    ...
    wash_trigger_event_init(&wash_trigger_event,
        ...
        "ownership-stop",
        "contract",
        device_runtime_private_runtime(system_context)->current_time_ms);
    ...
    TEST_ASSERT(device_runtime_private_runtime(system_context)->global_fault_present == false);
...
static int verify_recorder_owns_latest_projection(void)
{
    ...
    test_setup_system_context(&system_context, &driver_context);
    wash_session_abort(&device_runtime_private_runtime(system_context)->wash_session, RESULT_CODE_MANUAL_ABORT, "already-final", 77ul);
    ...
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_transition_record.reason_code, "projection-only") == 0);
...
static int verify_trigger_entry_owns_global_fault(void)
{
    ...
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->global_fault_code, "E_STOP") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->global_fault_reason, "ownership-fault") == 0);
    TEST_ASSERT(strcmp(device_runtime_private_runtime(system_context)->last_result_code, "accepted") == 0);
