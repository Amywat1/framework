...
static int verify_context_holds_shared_runtime_facts(void)
{
    ...
    pending_trigger_count_before = device_runtime_private_runtime(system_context)->pending_trigger_count;
    current_time_before = device_runtime_private_runtime(system_context)->current_time_ms;
    ...
    TEST_ASSERT(device_runtime_private_runtime(system_context)->pending_trigger_count == pending_trigger_count_before);
    TEST_ASSERT(device_runtime_private_runtime(system_context)->current_time_ms == current_time_before);
...
static int verify_public_lifecycle_preserves_bound_ports(void)
{
    ...
    test_setup_system_context(&system_context, &driver_context);
    program_repository_port_ptr = device_runtime_program_repository_port(system_context);
...
static int verify_release_clears_single_instance_and_invalidates_handle(void)
{
    ...
    TEST_ASSERT(device_runtime_pending_trigger_count(system_context) == 0u);
    TEST_ASSERT(device_runtime_current_time_ms(system_context) == 0ul);
    TEST_ASSERT(device_runtime_program_repository_port(system_context) == 0);
