...
/* 引擎 IO 后端（M8 机型：桥�?engine IO 接口到设备驱�?API�?*/
extern void engine_io_m8_register(void);
/* 方案加载器（JSON 格式实现注册�?engine_program_loader_port�?*/
...
/**
 * @brief  按引擎当前阶段行进方向注入限位信号，避免双限位同时触发组合报�?
 */
...
/* -------------------------------------------------------------------------
 * 辅助：等�?device_fsm 到达目标状态（最�?max_ms 毫秒�?
 * ------------------------------------------------------------------------- */
...
static void scenario_setup(void)
{
    ...
    /* 引擎 IO 后端：M8 机型桥接�?gantry/brush/water 设备驱动 */
    ...
    m8_signal_sim_set_lift_top(true);          /* 升降初始在上限位，跳�?homing 中的升降步骤 */
    m8_signal_sim_set_rear_lock_home(true);    /* 后轮锁初始在原点，满�?homing 退出条�?*/
    ...
    /* 注册并启动线程：event_dispatch + io_poll + wash_worker（已�?orchestrator 注册�?/
...
static void tc1_normal_complete(void)
{
    ...
    /* 等待洗车结束（prepare 2s + 7 个运动阶�?× ~100ms；余�?5s�?/
...
/* -------------------------------------------------------------------------
 * TC-3：运营停�?/ 恢复（STOP_OPERATION �?STOP �?RESUME_OPERATION �?IDLE�?
 * ------------------------------------------------------------------------- */
