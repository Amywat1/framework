...
sw_err_t m8_vfd_setup(void)
{
    ...
    cfg = (snack_vfd_backend_instance_cfg_t){
    ...
    ret = snack_vfd_backend_instance_init(HAL_VFD_BRUSH, &cfg);
    if (ret != SW_OK)
    {
    ...
    ret = snack_vfd_backend_instance_init(HAL_VFD_GANTRY, &cfg);
    if (ret != SW_OK)
    {
    ...
    ret = snack_vfd_backend_instance_init(HAL_VFD_FAN, &cfg);
    if (ret != SW_OK)
    {
