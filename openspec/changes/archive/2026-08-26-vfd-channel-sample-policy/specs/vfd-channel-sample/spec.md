...
每个已绑定实例 MUST 为故障码通道与电流通道各携带 `hal_vfd_channel_policy_t`。`OFF` 的 `period_ms` MUST 为 0；`BACKGROUND` 与 `FAST` 的 `period_ms` MUST 大于 0。`FAST` 未指定周期时 MUST 使用 20ms。绑定 MUST 允许任意多个实例的任意通道为 `FAST`。策略全为 `OFF` 时 MUST 按 `monitor_mask` 与旧周期字段映射：无对应位则为 `OFF`；电流周期等于 `HAL_VFD_FAST_CURRENT_PERIOD_MS` 则为周期 20ms 的 `FAST`；其余有位则为 `BACKGROUND`（周期 0 时用 2000ms）。非法策略 MUST 使 bind 失败。
...
#### Scenario: 旧 mask 映射为 FAST 电流

- **GIVEN** 通道策略均为 OFF，`monitor_mask` 含电流位，`current_period_ms` 为 `HAL_VFD_FAST_CURRENT_PERIOD_MS`
- **WHEN** bind 成功且实例进入 FWD，监测推进至少 1 拍
- **THEN** MUST 对该实例电流寄存器发起读，且 MUST NOT 在同一拍再读该实例故障码

#### Scenario: 无监测位则不发总线读

- **GIVEN** 策略全 OFF 且 `monitor_mask` 为 NONE，实例已 init
- **WHEN** 连续推进监测至少 3 拍
