# motor-fault-confirm Specification

## Purpose

本轴故障后能否再 `run`/`home` 由该电机 `confirm_faults` 按故障码决定：未列入即可续动（运动命令内部完成与显式 recover 相同的两步清除再启动），列入则需确认。默认位图 0 表示全部可续动。不读取 `alarm_registry`，不为清障单独扫轴。`fatal`、端口致命、ESTOP/`output_hold`、看门狗安全态不可被位图放行。
