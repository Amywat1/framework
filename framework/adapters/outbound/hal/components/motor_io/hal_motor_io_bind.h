 ...
 * @param  speed_ref  >0 正转�?0 反转�? 停止；绝对值为频率量纲（由注入方定义）
 * @param  ctx        注入方传入的上下文（�?vfd_id�?
 */
typedef sw_err_t (*hal_motor_set_speed_fn)(int speed_ref, void *ctx);

/**
 * @brief  挡位控制回调（VFD 挡位模式启动�?
 * @param  gear  正�?正转，负�?反转�?=停止；绝对值为挡位号（1=最低档�?
 * @param  ctx   注入方传入的上下�?
 */
...
{
    /**
     * @brief  频率速度输出回调；NULL 时退化为�?DO 方向控制
     * @note   �?VFD 频率模式的项目由机型 setup 注入；无 VFD 的项目保�?NULL
     */
