 ...
 * @brief   Snack MQTT 云适配器实现（命令下行 + 状态上报）
 * @author  HUWANGWEI
 * @date    2026-04-10
 */
...
/* 命令分发器（由 snack_cloud_command_adapter_init 注入） */
static snack_cloud_cmd_dispatch_fn_t s_cmd_dispatch = NULL;

/* 上报 JSON 构建器（由 snack_cloud_report_adapter_register 注入） */
...
static void mqtt_recv_cb(const char *msg)
{
    ...
    {
        LOG_WARN("snack_cloud: dispatch not registered");
        return;
    }
...
bool snack_cloud_command_adapter_init(snack_cloud_cmd_dispatch_fn_t dispatch)
{
    ...
    if (ds != NULL)
    {
        ...
        if ((ds->get(DEPLOY_KEY_TOPIC_UP, s_topic_up, sizeof(s_topic_up)) != SW_OK) ||
            (s_topic_up[0] == '\0'))
        {
            LOG_ERROR("snack_cloud: deploy config missing key=%s", DEPLOY_KEY_TOPIC_UP);
            return false;
        }
    }
    ...
    if (aliyun_mqtt_init(product_key, device_sn, device_secret) == 0)
    {
        ...
        LOG_INFO("snack_cloud: MQTT connected, sn=%s", device_sn);
        return true;
    }

    LOG_WARN("snack_cloud: MQTT init failed, running offline");
    return false;
}
...
static sw_err_t adapter_report(const cloud_report_payload_t *payload)
{
    ...
    {
        LOG_ERROR("snack_cloud: json build failed");
        return SW_ERR_PARAM;
    }
    if (net_mqtt_send(s_topic_up, buf) != 0)
    {
        LOG_WARN("snack_cloud: send failed");
        return SW_ERR_COMM;
    }
    return SW_OK;
}
...
void snack_cloud_report_adapter_register(snack_cloud_report_builder_fn_t builder)
{
    ...
    LOG_INFO("snack_cloud: report adapter registered");
}
