...
static void remove_active_at_locked(unsigned idx)
{
    ...
    {
        memmove(&s_active[idx], &s_active[idx + 1U],
                (s_active_count - idx - 1U) * sizeof(s_active[0]));
    }
    s_active_count--;
}
...
static bool evict_lowest_minor_locked(void)
{
    ...
    {
        if (((int)s_active[i].level < lowest) &&
            (s_active[i].response != RESP_STOP_IMMEDIATELY))
        {
...
static sw_err_t insert_active_locked(const alarm_def_t *def)
{
    ...
    if (s_active_count >= ALARM_ACTIVE_MAX)
    {
        if ((def->response == RESP_STOP_IMMEDIATELY) ||
            (def->level == ALARM_LEVEL_CRITICAL))
        {
            if (!evict_lowest_minor_locked())
            {
                if (!is_overflow_meta)
                ...
                LOG_ERROR("alarm_registry: active pool full, reject CRITICAL %06u",
                          (unsigned)def->code);
                return SW_ERR_OVERFLOW;
            }
        }
...
unsigned alarm_registry_pull_events(alarm_domain_event_t *buf, unsigned max)
{
    ...
    if (pulled > 0U)
    {
        memmove(s_pending, &s_pending[pulled],
                (s_pending_count - pulled) * sizeof(s_pending[0]));
