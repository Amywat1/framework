...
static bool resolve_var_name(const char *name, var_ctx_t *ctx)
{
    ...
    if ((ctx->catalog != NULL) &&
        name_in_list(name, ctx->catalog->signals, ctx->catalog->signal_count))
    {
        return true;
    }
...
static void validate_actions(const engine_action_t *acts, unsigned count,
                             var_ctx_t *ctx)
{
    for (unsigned i = 0U; (i < count) && ctx->ok; ++i)
    {
        ...
        if ((ctx->catalog != NULL) &&
            !name_in_list(acts[i].channel, ctx->catalog->outputs,
                          ctx->catalog->output_count))
        {
...
static void validate_step_signals(const engine_step_t *st, var_ctx_t *ctx)
{
    ...
    {
        if ((ctx->catalog != NULL) &&
            !name_in_list(st->trigger.signal, ctx->catalog->signals,
                          ctx->catalog->signal_count))
        {
    ...
    {
        if ((ctx->catalog != NULL) &&
            !name_in_list(st->done.signal, ctx->catalog->signals,
                          ctx->catalog->signal_count))
        {
                                 ...
                                 char *err, unsigned errsz)
{
    ...
    for (unsigned m = 0U; m < prog->marker_count; ++m)
    {
        ...
        if ((catalog != NULL) &&
            !name_in_list(mk->signal, catalog->signals, catalog->signal_count))
        {
        ...
        for (unsigned l = 0U; l < ph->lane_count; ++l)
        {
            ...
            for (unsigned s = 0U; s < lane->step_count; ++s)
            {
                ...
                if (st->type == ENGINE_STEP_CONTROL)
                {
                    ...
                    if ((catalog != NULL) &&
                        !name_in_list(st->output, catalog->outputs, catalog->output_count))
                    {
