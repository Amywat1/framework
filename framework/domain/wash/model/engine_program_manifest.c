...
static bool read_file_bytes(const char *path, uint8_t **out_buf, long *out_sz,
...
static sw_err_t hash_file_sha256_hex(const char *path, char out_hex[SHA256_HEX_LEN + 1U],
                                     long *out_size, char *err, unsigned errsz)
{
    uint8_t       *buf = NULL;
    long           sz  = 0L;
    sha256_ctx_t   ctx;
