...
#include "machines/m8/config/m8_motor_binding.h"
...
typedef struct {
    ...
    sw_err_t  (*init)(motor_executor_t *exec); /**< 绑定函数 */
} m8_motor_domain_entry_t;

static sw_err_t domain_init_brush(motor_executor_t *exec)
{
    return brush_init(exec, M8_BIND_BRUSH_SIDE, M8_BIND_BRUSH_TOP);
}

static sw_err_t domain_init_gantry(motor_executor_t *exec)
{
    return gantry_init(exec, M8_BIND_GANTRY);
}

static sw_err_t domain_init_lift(motor_executor_t *exec)
{
    return lift_init(exec, M8_BIND_LIFT);
}

static sw_err_t domain_init_rear_lock(motor_executor_t *exec)
{
    return rear_lock_init(exec, M8_BIND_REAR_LOCK);
}

static sw_err_t domain_init_fan(motor_executor_t *exec)
{
...
{
    motor_executor_t *exec = m8_motor_exec_get();
    sw_err_t          ret;
    size_t            i;
