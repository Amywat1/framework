...
bool m8_mqtt_command_parse(const char *json_str, cmd_t *out_cmd)
{
    ...
    action_item = cJSON_GetObjectItem(root, M8_CMD_JSON_FIELD_ACTION);
    ...
    if (strcmp(action, M8_CMD_JSON_ACTION_START_WASH) == 0)
    {
        out_cmd->type = CMD_START_WASH;
        cJSON *mode_item = cJSON_GetObjectItem(root, M8_CMD_JSON_FIELD_MODE);
    ...
    else if (strcmp(action, M8_CMD_JSON_ACTION_STOP_WASH) == 0)
    {
        out_cmd->type = CMD_STOP_WASH;
        ok = true;
    }
    else if (strcmp(action, M8_CMD_JSON_ACTION_STOP_OPERATION) == 0)
    {
        out_cmd->type = CMD_STOP_OPERATION;
        ok = true;
    }
    else if (strcmp(action, M8_CMD_JSON_ACTION_RESUME_OPERATION) == 0)
    {
        out_cmd->type = CMD_RESUME_OPERATION;
        ok = true;
    }
    else if (strcmp(action, M8_CMD_JSON_ACTION_RESET_FAULT) == 0)
    {
        out_cmd->type = CMD_RESET_FAULT;
        ok = true;
    }
    else if (strcmp(action, M8_CMD_JSON_ACTION_HOME_DEVICE) == 0)
    {
    ...
    {
        LOG_WARN("m8_cmd_parser: unknown action=%s", action);
    }
